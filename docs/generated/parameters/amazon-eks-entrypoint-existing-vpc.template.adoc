...
(`MaxNumberOfNodes`)|`3`|The maximum number of Amazon EKS node instances. The default is three nodes.|Instance family
...
(`NodeGroupOS`)|`Amazon Linux 2`|The operating system to be used for the node instances. Note that if you choose "Windows" an additional Amazon Linux node group is also created.
|===

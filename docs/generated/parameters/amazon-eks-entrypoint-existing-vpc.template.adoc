...
(`HttpProxy`)|`**__Requires input__**`|[OPTIONAL] HTTP(S) proxy configuration, if provided all worker nodes and pod egress traffic will use this proxy. Example: http://10.101.0.100:3128/.
|===
...
(`AdditionalEKSAdminRoleArn`)|`**__Requires input__**`|[OPTIONAL] IAM role Amazon Resource Name (ARN) to be granted admin access to the EKS cluster.|Fargate namespaces
...
(`NewRelicLicenseKey`)|`**__Requires input__**`|If New Relic is enabled, this must be provided. For more information see https://docs.newrelic.com/docs/accounts/install-new-relic/account-setup/license-key .
|===
.Kubernetes add-ins
[width="100%",cols="16%,11%,73%",options="header",]
|===
